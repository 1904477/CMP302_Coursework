...
void AUFirstPersonCharacter::changeBullCountDown()
{
	m_changeBulletVar--;
	if (m_changeBulletVar == 0)
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Gravitational Bullet Selected"));
	if (m_changeBulletVar == 1)
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Explosive Bullet Selected"));
	if (m_changeBulletVar == 2)
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Zero-Gravity Bullet Selected"));
}
void AUFirstPersonCharacter::changeBullCountUp()
{
	m_changeBulletVar++;
	if (m_changeBulletVar == 0)
		GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Gravitational Bullet Selected"));
...
void AUFirstPersonCharacter::BlackHole()
{
	if (blackHoleSpawnable == true)
	{
		...
		if (m_changeBulletVar == 0)
		{
			GetWorldTimerManager().SetTimer(Cooldown, this, &AUFirstPersonCharacter::TimerFunc, 3, false);
			...
			GetWorld()->SpawnActor<AUGravity_Projectile>(SpawnLocation, SpawnRotation, SpawnInfo);
			blackHoleSpawnable = false;
		}
		if (m_changeBulletVar == 1)
		{
			GetWorldTimerManager().SetTimer(Cooldown, this, &AUFirstPersonCharacter::TimerFunc, 3, false);
			...
			const FVector SpawnLocation = (GetWorld()->GetFirstPlayerController()->GetPawn()->GetActorLocation() + SpawnRotation.RotateVector(GunOffset));
			FActorSpawnParameters SpawnInfo;
			GetWorld()->SpawnActor<AUExplosion_Projectile>(SpawnLocation, SpawnRotation, SpawnInfo);
			blackHoleSpawnable = false;
		}
		if (m_changeBulletVar == 2)
		{
			...
			const FVector SpawnLocation = (GetWorld()->GetFirstPlayerController()->GetPawn()->GetActorLocation() + SpawnRotation.RotateVector(GunOffset));
			FActorSpawnParameters SpawnInfo;
			GetWorld()->SpawnActor<AUFloatingBullet>(SpawnLocation, SpawnRotation, SpawnInfo);
			blackHoleSpawnable = false;
		}
	}
}
